...
/// (Bluetooth rename, factory reset) requires a real recovery step or is
/// destructive, so it shouldn't be as casually reachable as the day-to-day
/// controls on Paràmetres. Full-config export/import lives on the "Eines"
/// screen instead (see `ToolsScreen`) — it's a config-management action,
/// not a device-identity/recovery one.
...
class _PinSectionState extends ConsumerState<_PinSection> {
  ...
  void initState() {
    ...
    // Retries a few times a second apart — see _BluetoothNameSectionState's
    // doc for why a single request here isn't reliable enough (confirmed
    // on real hardware: silently lost on at least one Android phone right
    // after connecting, while working every time on another in the same
    // session). Harmless to repeat: a duplicate reply just re-sets the
    // same value.
    var retriesLeft = 4;
